...
sequenceDiagram
    ...
    Note over B: CONFLICT (content): README.md<br/>Auto-merging 실패
    B->>B: README.md 충돌 표시 확인 및 해결<br/>(<<<<<<< / ======= / >>>>>>> 제거)
    ...
    Note over A,G,B: 최종적으로 작업자 A, 작업자 B, GitHub가<br/>동일한 최신 커밋 상태로 동기화됨
```

## 참고: 핵심 흐름 요약
- 다른 작업자의 변경은 `git fetch` 후 `git merge`를 실행해야만 로컬에 반영된다.
